 ...
 ********************************************************/
    ...
    IndexPQ::IndexPQ(int d, size_t M, size_t nbits, MetricType metric) : Index(d, metric), pq(d, M, nbits)
    {
        is_trained = false;
...
#pragma omp parallel for
            ...
            {
                pq.compute_code_from_distance_table(dis_tables + qi * pq.M * pq.ksub,
...
#pragma omp parallel for reduction(+ \
        ...
        for (idx_t qi = 0; qi < n; qi++)
        {
            ...
            if (search_type == ST_polysemous)
            {
                ...
                {
                case 4:
                    n_pass += polysemous_inner_loop<HammingComputer4>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    break;
                case 8:
                    n_pass += polysemous_inner_loop<HammingComputer8>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    break;
                case 16:
                    n_pass += polysemous_inner_loop<HammingComputer16>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    break;
                case 32:
                    n_pass += polysemous_inner_loop<HammingComputer32>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    break;
                case 20:
                    n_pass += polysemous_inner_loop<HammingComputer20>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    break;
                default:
                    if (pq.code_size % 8 == 0)
                    {
                        n_pass += polysemous_inner_loop<HammingComputerM8>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    }
                    else if (pq.code_size % 4 == 0)
                    {
                        n_pass += polysemous_inner_loop<HammingComputerM4>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    }
                    else
                    {
                        FAISS_THROW_FMT(
            ...
            else
            {
                ...
                {
                case 8:
                    n_pass += polysemous_inner_loop<GenHammingComputer8>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    break;
                case 16:
                    n_pass += polysemous_inner_loop<GenHammingComputer16>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    break;
                case 32:
                    n_pass += polysemous_inner_loop<GenHammingComputer32>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    break;
                default:
                    if (pq.code_size % 8 == 0)
                    {
                        n_pass += polysemous_inner_loop<GenHammingComputerM8>(*this, dis_table_qi, q_code, k, heap_dis, heap_ids);
                    }
                    else
                    {
                        FAISS_THROW_FMT(
 ...
 ******************************************/
    ...
    void IndexMultiPQ::train(idx_t n, const float *x)
    {
        mpq.train_low(n, x);
    }
    ...
    void IndexMultiPQ::add_priors(idx_t n, std::vector<float> x)
    {
        priors = x;
        high_indexes.clear();
        high_precision_lookup.clear();
        idx_t cnt = 0;
        ...
        for (int i = 0; i < x.size(); i++)
        {
            if (x[i] > threshold)
            {
                high_indexes.push_back(i);
                high_precision_lookup.insert(std::pair<idx_t, idx_t>(i, cnt));
                cnt++;
            }
        }
    }

    void IndexMultiPQ::add(idx_t n, const float *x)
    {
        codes_low.resize(n * mpq.code_size_low);
    ...
    void IndexMultiPQ::reconstruct_n(idx_t i0, idx_t ni, float *recons) const
    {
        for (idx_t i = 0; i < ni; i++)
        {
    ...
    void IndexMultiPQ::reconstruct(idx_t key, float *recons) const
    {
 ...
 */
        ...
        struct SemiSortedArray
        {
            ...
            void grow(int next_k)
            {
                ...
                { // full sort of remainder of array
                    ArgSort<T> cmp = {x};
 ...
 ******************************************/
    ...
    MultiIndexQuantizer::MultiIndexQuantizer(int d,
                                             size_t M,
                                             size_t nbits) : Index(d, METRIC_L2), pq(d, M, nbits)
    {
        is_trained = false;
        pq.verbose = verbose;
    }
 ...
 ******************************************/
    ...
    MultiIndexQuantizer2::MultiIndexQuantizer2(
        ...
        Index **indexes) : MultiIndexQuantizer(d, M, nbits)
    {
    ...
    MultiIndexQuantizer2::MultiIndexQuantizer2(
        ...
        Index *assign_index_1) : MultiIndexQuantizer(d, 2, nbits)
    {
        FAISS_THROW_IF_NOT_MSG(
