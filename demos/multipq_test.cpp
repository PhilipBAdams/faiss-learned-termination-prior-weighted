...
int main(int argc, char* argv[])
{
	...
	if (argc != 11)
	{
		 printf("Usage: %s data_file train_low_file train_high_file query_file prior_file gt_file threshold M nbits_low nbits_high\n", argv[0]);
		 exit(1);
	}
...

	{
		 ...
		 assert(d == dp || !"train set and prior set have different dimension");
...

	{
		 ...
		 size_t nq2;
		 int *gt_int = ivecs_read(gt_file, &k, &nq2);
